...
testCreateTableAndInsert (void)
{
    ...
    {
        //printf("testCreateTableAndInsert insert ------------- %d times\n",i);
	...
	for(i = 0; i < 1000; i++)
	{
        //printf("testCreateTableAndInsert compare ------------- %d times\n",i);
...
testMultipleScans(void)
{
	...
	for(i = 0; i < numInserts; i++)
	{
        //printf("testMultipleScans insert ------------- %d times\n",i);
		r = fromTestRecord(schema, inserts[i]);
		TEST_CHECK(insertRecord(table,r));
		rids[i] = r->id;
        freeRecord(r);  // Added: Summer 2021
...
testUpdateTable (void)
{
	...
	for(i = 0; i < numInserts; i++)
	{
        //printf("testUpdateTable insert ---------------%d times\n",i);
		r = fromTestRecord(schema, inserts[i]);
		TEST_CHECK(insertRecord(table,r));
		rids[i] = r->id;
        freeRecord(r);   // Added Summer 2021
	...
	{
       // printf("testUpdateTable compare ---------------%d times\n",i);
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid, r));
