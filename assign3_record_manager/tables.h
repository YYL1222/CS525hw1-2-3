...
// Contains Linked Lists of PageNumbers for full pages (dataPages) and pages with room for more records (freePages).
// Both Linked Lists are bidirectional and have an empty head (rel->freePages->pageNum == rel->dataPages->pageNum == NO_PAGE).
// Contains the BufferPool associated with the table. A new one is created when the table is opened, and it is shut down when the table is closed.
